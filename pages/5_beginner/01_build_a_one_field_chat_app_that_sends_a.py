...
st.set_page_config(page_title="1 - Build a one-field chat app that sends a ...", page_icon="💬")

st.title("💬 Build a one-field chat app that sends a ...")
st.write("""Build a one-field chat app that sends a single user message to Ollama and displays the reply.""")
...
if prompt := st.chat_input("Type your message..."):
    ...
    try:
        ...
        response = get_chat_response(
            model=model,
            user_message=prompt,
            history=st.session_state["chat_1_messages"][:-1] if len(st.session_state["chat_1_messages"]) > 1 else None,
