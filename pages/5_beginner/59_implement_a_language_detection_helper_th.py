    ...
    if text.strip():
        ...
        try:
            ...
            response = get_chat_response(
                model="llama3.2",
                user_message=text,
                system_prompt="You are a language detection expert. Detect the language of the text. Respond with a JSON object containing 'classification' (the detected language) and 'confidence' (a number between 0.7 and 1.0). Example: {\"classification\": \"English\", \"confidence\": 0.95}",
                temperature=0.3,
            )
            try:
